...
public class ReportGenerationDAOImpl implements IReportGenerationDAO {
  ...
  public List<Integer> getAutoCompleteInfo(int employeeId) {
    ...
    String hql = "select employeeId from com.caprusit.ems.domain.Employee where employeeId like '"
        + employeeId + "%'";
  ...
  public List<String> getAutoCompleteInfo(String employeeId) {
    ...
    String hql = "select firstName, lastName from com.caprusit.ems.domain.Employee where firstName like '"
        + employeeId + "%'";
    Query query = HibernateSessionUtility.getHibernateSession().createQuery(hql);
  ...
  @SuppressWarnings("unchecked")
  public List<Attendance> getEmployeeWorkingDetailsByIdAndDate(int employeeId,
      Date attendanceDate) {
  ...
  @SuppressWarnings("unchecked")
  public List<Attendance> getEmployeeWorkingDetailsByDates(int employeeId, Date fromDate,
      Date toDate) {
  ...
  public List<Attendance> getEmployeesReportBetweenDates(Date fromDate, Date toDate) {
    ...
    Criteria crit = HibernateSessionUtility.getHibernateSession().createCriteria(Attendance.class)
        ...
        .addOrder(Order.asc("attendanceDate"));
    List<Attendance> result = crit.list();
  ...
  public List<Attendance> getTodayPresentAttendance() {
    ...
    Criteria toDayReportCriteria = HibernateSessionUtility.getHibernateSession()
        .createCriteria(Attendance.class)
        .add(Restrictions.and(Restrictions.eq("attendanceDate", Calendar.getInstance().getTime()),
            Restrictions.eq("dayIndicator", EmsConditions.EMPLOYEE_PRESENT_STATUS)));
    /*
  ...
  public List<Attendance> getTodayLeaveAttendance() {
    return HibernateSessionUtility.getHibernateSession().createCriteria(Attendance.class)
        .add(Restrictions.and(Restrictions.eq("attendanceDate", Calendar.getInstance().getTime()),
            Restrictions.eq("dayIndicator", EmsConditions.EMPLOYEE_LEAVE_STATUS)))
        .list();
  }
  ...
  public int getNumberOfEmployees() {
    ...
    Criteria noOfEmployeeCriteria = HibernateSessionUtility.getHibernateSession()
    ...
    logger.info("total number of employees: " + noOfEmployeeList + "class = "
        + noOfEmployeeList.get(0).getClass());
  ...
  public int getNumberOfActiveEmployees() {
    List<Long> countList = HibernateSessionUtility.getHibernateSession()
