...
public class SecurityDAOImpl implements ISecurityDAO {
	...
	/**
	 * This method is to get admin details like first name and last name and
	 * reads current admin password
	 */
	...
	public List<Object> forgotPassword(int adminId) {
		...
		try {
			...
			Query query = HibernateSessionUtility.getHibernateSession()
					.createQuery(verifyEmailId);
	...
	public EncryptedEmployee getEmployeeCurrentPassword(int employeeId) {
		...
		EncryptedEmployee employee = (EncryptedEmployee) HibernateSessionUtility
				.getHibernateSession().get(EncryptedEmployee.class, employeeId);

		return employee;
	}

	/**
	 * This method is to change employee password updates old password with new
	 * password
	 */
	@Override
	...
	public List<Integer> getAdminRoleId() {
		...
		return	HibernateSessionUtility
				.getHibernateSession()
