...
public class LoginFailedDAOImple implements LoginFailedAttemptsDAO{
	...
	public int checkAttemptsCount(int adminId) {
		...
		SQLQuery qry=HibernateSessionUtility.getHibernateSession().createSQLQuery("select attemptcount from PRAKASH.employeePassword_table where employeeid="+adminId);
	...
	public void incrementAttemptCount(int adminId) {
		...
		SQLQuery qry=HibernateSessionUtility.getHibernateSession().createSQLQuery("update PRAKASH.employeePassword_table set attemptcount=attemptcount+1 where employeeid="+adminId);
		qry.executeUpdate();
	}
	...
	public int LockUser(int adminId) {
		...
		SQLQuery qry=HibernateSessionUtility.getHibernateSession().createSQLQuery("update PRAKASH.employeePassword_table set locked=1 where employeeid="+adminId);
	...
	public int setDefualtAttemptCount(int adminId) {
		...
		SQLQuery qry=HibernateSessionUtility.getHibernateSession().createSQLQuery("update PRAKASH.employeePassword_table set attemptcount=0 where employeeid="+adminId);
		int res=qry.executeUpdate();
	...
	public String[] getMailID(int adminId) {
		...
		SQLQuery qry=HibernateSessionUtility.getHibernateSession().createSQLQuery("select firstname,lastname,emailid from prakash.employee_table where employeeid="+adminId);
