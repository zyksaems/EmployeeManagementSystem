...
public class SecurityController {
  ...
  /**
   * This method is for admin login functionality Takes admin object as request body Returns 1 on
   * successful login If admin credentials are correct and creates a session and sets admin ID as
  ...
  /**
   * This method is for admin home page If admin id is there in session returns admin home page
  ...
  /**
   * This method is for Employee home page If employee id is there in session returns employee home
   * page otherwise returns error page.
   */
  ...
  public String getEmployeeHomePage(HttpServletRequest request) {
    ...
    return (HttpSessionUtility.verifySession(request, "employeeId")) ? "EmployeeDashBoard"
        : "EmsHomePage";
  }

  /**
   * getChangePasswordPage() method will display ChangePasswordPage when we click on ChangePassword
   * button in AdminDashboard.
   */
  ...
  public String getChangePasswordPage(HttpServletRequest request) {
    ...
    return (HttpSessionUtility.verifySession(request, "adminId")) ? "ChangePassword"
        : "EmsHomePage";
  }

  /**
   * This method is for admin logout functionality Returns 1 on successful logout.
   */
  ...
  public @ResponseBody int adminLogout(HttpServletRequest request) {
    ...
    if (session != null) {
      logger.info("invalidating session (employee ID)" + session.getAttribute("employeeId") + ""
  ...
  /**
   * forgotPassword() method takes adminId and emailId as parameter and after validating both input
   * , it returns success message such as Email Sent Successfully or error message such as You
   * entered incorrect adminId or EmailId on the browser.
   */
  @SuppressWarnings("unchecked")
  @RequestMapping(value = "/forgotPasswordHome", method = RequestMethod.POST)
  public @ResponseBody int forgotPassword(HttpServletRequest request,
      @RequestParam("id") Integer adminId, @RequestParam("email") String emailId) {
    ...
    String url = "http://" + request.getServerName() + ":" + request.getServerPort()
        + "/EmployeeManagementSystemNew/getResetPasswordPage.do?id=" + adminId + "&&pas="
  ...
  /**
   * This method is returns the JSP page(ModelAndView) to administrator In this JSP page admin can
   * reset his password.
   */
  @SuppressWarnings("unchecked")
  ...
  public ModelAndView getResetPAsswordPage(HttpServletRequest request,
      @RequestParam("id") int adminId, @RequestParam("pas") long milliSeconds) {
    logger.info("admin id received for reset password  :" + adminId);
    ServletContext servletContext = request.getSession().getServletContext();
    long currentMilliSeconds = Calendar.getInstance().getTimeInMillis();
    Set<Integer> resetAdminIdSet = (Set<Integer>) servletContext
        .getAttribute("resetPaswordAdminIdList");
    ModelAndView resetAdminModelAndVlew = (resetAdminIdSet != null
        && resetAdminIdSet.contains(adminId)
        && (currentMilliSeconds <= milliSeconds + passwordLinkValidTime * 60000))
            ? new ModelAndView("ResetPasswordPage", "resetPasswordAdminId", adminId)
            : new ModelAndView("ResetPasswordPage", "errorMsg", "Sorry, This link is expired!!");
    logger.info("model and view created for reset password: " + resetAdminModelAndVlew);
    // logger.info("condition for time checking: "+(currentMilliSeconds <=
    // milliSeconds+passwordLinkValidTime*60000 ));
    // logger.info("condition for time checking currentTime:- "+currentMilliSeconds );
    // logger.info("condition for time checking sent+: ++ "+ (currentMilliSeconds <=
    // milliSeconds+passwordLinkValidTime*60000) );
  ...
  /**
   * This method is to reset the admin password returns 1 after successful reset.
   */
  ...
  /**
   * changePassword() method implementation This method takes current password and new password as
   * parameter and after checking all the conditions it returns either a successful or an error
   * message to the browser.
   */
  @RequestMapping(value = "/changePassword.do", method = RequestMethod.POST)
  public @ResponseBody String changePassword(HttpServletRequest request,
      @RequestParam("cpwd") String oldPassword, @RequestParam("npwd") String newPassword) {
  ...
  /**
   * changeEmployeePassword() method implementation This method takes current password and new
   * password as parameter and after checking all the conditions it returns either a successful or
   * an error message to the browser.
   */
  @RequestMapping(value = "/changeEmployeePassword", method = RequestMethod.POST)
  public @ResponseBody int changeEmployeePassword(
      @RequestBody ChangePasswordRequest changePasswordData) {
  ...
  public String viewEmployeeLeavePage(HttpServletRequest request) {
    ...
    return (HttpSessionUtility.verifySession(request, "employeeId")) ? "EmployeeChangePassword"
        : "EmsHomePage";
