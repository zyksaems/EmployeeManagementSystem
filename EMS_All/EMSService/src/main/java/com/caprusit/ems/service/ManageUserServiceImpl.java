...
public class ManageUserServiceImpl implements IManageUserService {
  ...
  /**
   * @param emp
   *          Employee object to be saved into database
   * @param milliseconds
   *          Date of birth of employee in milliseconds
   * @return int returns 1 if employee successfully saved 0 if employee not saved successfully
   */
  @Transactional(rollbackFor = SQLException.class)
  public int addSingleEmployee(Employee emp, String milliseconds) {
    ...
    if (id == 1) {
      ...
      encEmp.setEncryptedPassword(
          EncryptionUtility.encryptString(String.valueOf(emp.getEmployeeId())));
