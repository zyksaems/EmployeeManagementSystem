...
public class ReportGenerationServiceImpl implements IReportGenerationService {
  ...
  private int workingHoursPerDay = EmsConditions.WORKING_HOURS_PER_DAY,
      daysPerMonth = EmsConditions.WORKING_DAYS_PER_MONTH;
  ...
  public String getEmployeeWorkingDetailsByIdAndDate(int employeeId, Date attendanceDate) {
    logger.info(
        "Inside ReportGenerationServiceImpl getEmployeeWorkingDetailsByIdAndDate(-,-) method ");
    ...
    List<Attendance> listOfEmpReport = reportGenerationDAO
        .getEmployeeWorkingDetailsByIdAndDate(empId, attendanceDate);
  ...
  public String getEmployeeWorkingDetailsByDates(int employeeId, Date fromDate, Date toDate) {
    ...
    List<Attendance> listOfEmpReport = reportGenerationDAO.getEmployeeWorkingDetailsByDates(empId,
        fromDate, toDate);
    Map<String, Object> employeeReportMap = new HashMap<String, Object>();
    employeeReportMap.put("empId", empId);
    employeeReportMap.put("empName", employeeName);
  ...
  public String getSingleEmployeeDetailsByEmpName(String employeeId) {
    ...
    List<Object> empData = reportGenerationDAO.getSingleEmployeeDetailsByEmpName(firstName,
        lastName);
  ...
  public String getReportByNameDates(String employeeId, Date fromDate, Date toDate) {
    ...
    List<Attendance> listOfEmpReport = reportGenerationDAO.getReportByNameDates(empId, fromDate,
        toDate);
    Map<String, Object> employeeReportMap = new HashMap<String, Object>();
    employeeReportMap.put("empId", empId);
    employeeReportMap.put("empName", employeeName);
  ...
  /**
   * This method is to get today attendance details like absentees and Presenties details
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  ...
  public String getDailyReportOfIndividual(int employeeId, Date attendanceDate) {
    ...
    Map<String, Object> allMap = reportGenerationDAO.getDailyReportOfIndividual(employeeId,
        attendanceDate);
  ...
  /**
   * This method is to calculate individual employee annual productivity based on given year
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getEmployeeAnnualProductivity(int employeeId, int year) {
    ...
    logger.info("employee monthly report year start date : " + YearstartDate + "  year end date: "
        + yearEndDate);
    List<Attendance> employeeMonthlyData = reportGenerationDAO
        .getEmployeeWorkingDetailsByDates(employeeId, YearstartDate, yearEndDate);
    listSize = employeeMonthlyData.size();
    logger.info("attendance details received for monthly individual employee results: "
        + employeeMonthlyData);
  ...
  /**
   * This method is to calculate All employee annual productivity based on given year
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getAllEmployeeAnnualProductivity(int year) {
    ...
    double[] workingHoursArray = new double[endIndex];
    double[] nonWorkingHoursArray = new double[endIndex];
    List<Attendance> allEmployeeMonthlyData = reportGenerationDAO
        .getEmployeesReportBetweenDates(yearstartDate, yearEndDate);
    // calculate working hours pass 2 as parameter for month wise calculation
   ...
   * 
   * @param workedHoursArray
   *          double type array to store calculated worked hours of employee
   * @param attendanceDetailsList
   *          List of attendance from which we can calculate worked hours
   * @param typeOfCalculation
   *          It defines type of calculation either month or week. For week calculation pass 1 and
   *          for month pass 2
   */
  private void calculateWorkingHours(double[] workedHoursArray,
      List<Attendance> attendanceDetailsList, int typeOfCalculation) {
  ...
  /**
   * This method is to calculate employee non working hours from given worked hours and actual work
   * hours values
   * 
   * @param nonWorkingHoursArray
   *          double type array to store calculated non working hours
   * @param actualWorkHours
   *          actual work hours of employee
   * @param workedHoursArray
   *          double type array worked hours from which we can calculate non working hours
   */
  private void calculateNonWorkingHours(double[] nonWorkingHoursArray, double actualWorkHours,
      double[] workedHoursArray) {
    double workedHours;
  ...
  /**
   * getEmployeeReportForWeekByIdAndWeekDate(-,-) method takes integer employeeId and string
   * weekDate as parameters. call getEmployeeReportForWeekByIdAndWeekDate(-,-) method to get
   * fromDate , toDate and call EmployeeReportGenerationDAO getEmployeeWorkingDetailsByDates(-,-,-)
   * method to get employee working details for particular week.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getEmployeeReportForWeekByIdAndWeekDate(int employeeId, String weekDate)
      throws Exception {

    logger.info(
        "inside ReportGenerationServiceImpl getEmployeeReportForWeekByIdAndWeekDate(-,-) method");
    ...
    /*
     * Calling getFromDateAndToDateFromWeekDate(-) method to get fromDate and toDate based on given
     * string weekDate
     */
    ...
    List<Attendance> weeklyWorkingDetails = reportGenerationDAO
        .getEmployeeWorkingDetailsByDates(employeeId, startDate, endDate);
  ...
  /**
   * getAllEmployeeReportForWeekByWeekDate(-,-) method take string weekDate as parameters. call
   * getEmployeeReportForWeekByIdAndWeekDate(-) method to get fromDate , toDate and call
   * EmployeeReportGenerationDAO getEmployeesReportBetweenDates(-,-) method to get employee working
   * details for particular week.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getAllEmployeeReportForWeekByWeekDate(String weekDate) throws Exception {

    logger.info(
        "Inside ReportGenerationServiceImpl getAllEmployeeReportForWeekByWeekDate(-) method	");
    ...
    return JsonUtility
        .convertToJson(reportGenerationDAO.getEmployeesReportBetweenDates(startDate, endDate));
  }

  /**
   * getEmployeeReportForMonthByIdAndMonth(-,-) method takes int employeeId and string month as
   * parameters , and we will convert String month to java.util.Date and send to
   * EmployeeReportGenerationDAO getEmployeeWorkingDetailsByDates(-,-) method to get employee
   * working details for particular month.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getEmployeeReportForMonthByIdAndMonth(int employeeId, String month) {

    logger.info(
        "Inside ReportGenerationServiceImpl getEmployeeReportForMonthByIdAndMonth(-,-) method	");
    ...
    List<Attendance> monthlyWorkingDetails = reportGenerationDAO
        .getEmployeeWorkingDetailsByDates(employeeId, fromDate, toDate);
  ...
  /**
   * getAllEmployeeReportForMonthByMonth(-) method take string month as parameters , and we will
   * convert String month to java.util.Date and send to EmployeeReportGenerationDAO
   * getEmployeesReportBetweenDates(-,-) method to get employee working details for particular
   * month.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getAllEmployeeReportForMonthByMonth(String month) {
    ...
    logger
        .info("Inside ReportGenerationServiceImpl getAllEmployeeReportForMonthByMonth(-) method	");
    ...
    return JsonUtility
        .convertToJson(reportGenerationDAO.getEmployeesReportBetweenDates(fromDate, toDate));
  }

  /**
   * getEmployeeReportForYearByIdAndYear(-,-) method takes int employeeId and string year as
   * parameters , and we will convert String year to java.util.Date and send to
   * EmployeeReportGenerationDAO getEmployeeWorkingDetailsByDates(-,-,-) method to get employee
   * working details for particular month.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getEmployeeReportForYearByIdAndYear(int employeeId, String year) {
    ...
    logger.info(
        "Inside ReportGenerationServiceImpl getEmployeeReportForYearByIdAndYear(-,-) method	");
    ...
    List<Attendance> annuallyWorkingDetails = reportGenerationDAO
        .getEmployeeWorkingDetailsByDates(employeeId, startDate, endDate);
  ...
  /**
   * getAllEmployeeReportForYearByYearDate(-) method take string yearDate as parameters , and we
   * will convert String yearDate to java.util.Date and send to EmployeeReportGenerationDAO
   * getEmployeesReportBetweenDates(-,-) method to get employee working details for particular
   * month.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
   ...
   * 
   * @param employeeId
   *          employee id for productivity
   * @param weekDate
   *          week in string format like: 2016-W09
   * @throws ParseException
   *           if unable to parse given week
   * @return String JSON string of Map object
   */
  @SuppressWarnings("unchecked")
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getWeeklyProductivityOfEmployeeByIdAndWeek(int employeeId, String weekDate)
      throws ParseException {

    logger.info(
        "inside ReportGenerationServiceImpl getWeeklyProductivityOfEmployeeByIdAndWeek(-,-) method");
    ...
    List<Attendance> workingDetails = reportGenerationDAO
        .getEmployeeWorkingDetailsByDates(employeeId, weekStartDate, weekEndDate);
   ...
   * 
   * @param weekDate
   *          string format of week like like: 2016-W09
   * @return String JSON string of Map object
   */
  @SuppressWarnings("unchecked")
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getWeeklyProductivityOfAllEmployeeByWeek(String weekDate) {
    ...
    logger
        .info("inside ReportGenerationServiceImpl getWeeklyReportOfAllEmployeeByWeek(-,-) method");
    Date startDate = null, endDate = null;
    Map<String, Date> dateValues = getFromDateAndToDateFromWeekDate(weekDate);
    startDate = dateValues.get("startDate");
    endDate = dateValues.get("endDate");
    ...
    List<Attendance> workingDetails = reportGenerationDAO.getEmployeesReportBetweenDates(startDate,
        endDate);
  ...
  /**
   * This getFromDateAndToDateFromWeekDate(-) method provides fromDate and toDate in java.util.Date
   * format based on given weekDate of String type.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  ...
  /*
   * getMonthlyProductivityOfEmployeeByIdAndMonth(-,-) method will take employeeId, month as
   * parameters and based on given month , we calculate firstDate and lastDate of the month. By
   * passing employeeId, firstDate, lastDate to the getEmployeeWorkingDetailsByDates(-,-,-) method
  ...
  public String getMonthlyProductivityOfEmployeeByIdAndMonth(int employeeId, String month) {
    logger.info(
        "Inside ReportGenerationServiceImpl getMonthlyProductivityOfEmployeeByIdAndMonth(-,-) method	");
  ...
  /*
   * getMonthlyProductivityOfAllEmployeeByMonth(-,-) method will take month as parameter and based
   * on given month , we calculate firstDate and lastDate of the month. By passing firstDate,
   * lastDate to the getEmployeesReportBetweenDates(-,-) method get the working details of all
   * employee.
   */
  @Transactional(rollbackFor = HibernateException.class, readOnly = true)
  public String getMonthlyProductivityOfAllEmployeeByMonth(String month) {
    logger.info(
        "Inside ReportGenerationServiceImpl getMonthlyProductivityOfAllEmployeeByMonth(-,-) method	");
    ...
    List<Attendance> employeeMonthlyData = reportGenerationDAO
        .getEmployeesReportBetweenDates(fromDate, toDate);
    listSize = employeeMonthlyData.size();
    logger.info(
        "attendance details received for monthly all employee results: " + employeeMonthlyData);
  ...
  /**
   * This method is to verify end date is greater than current date,if true find outs array index of
   * week/months array based on given date
   * 
   * @param typeOfIndex
   *          type if index we want to calculate for week array pass 1 and for months array pass 2
   * @param endDate
   *          date,from which we can get index(if given endDate is > current date).
   * @return index of given end date,based on given typeOfIndex 1/2. if we pass other than 1/2 for
   *         typeOfIndex returns -2 and if endDate is less than current date returns -1
   */
