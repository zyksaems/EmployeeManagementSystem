	...
	public void  runSchedulerToRemindEmployees(){
		...
		logger.info("running not logout scheduler at: "+new Date());			
				...
				for(Integer empId: li){
					...
					logger.info("SchedulerDaoObjectsUtility.emailUtility.  object"+SchedulerDaoObjectsUtility.emailUtility);
					SchedulerDaoObjectsUtility.emailUtility.sendMail(emp.getEmailId(), message, emp.getFirstName()+" "+emp.getLastName(),"Alert from EMS");							
